...
warning('off','all')
warning
load('D:\Materials\revisits_WorldView.mat');
revisits_array=fliplr(revisits_WorldView);
...
SatName='WorldView';
numSeeds=10000;
Component_Type=1;%1 = Space-Grade, 2 = Space-Tolerant, 3 = COTS - only used for SEUs

%Spacecraft and Orbit Parameters
sc_length=2.5;        %spacecraft length in meters
sc_width=2.5;         %spacecraft width in meters
sc_mass=2500;         %spacecraft mass in kg
...
image_cap=1000000;    %imaging capacity per satellite per day (km^2)
manufac_time=30;      %manufacturing time per satellite in months
...
op_orbit_pop=4;       %number of satellites in constellation at t=0
...
%Cost Parameters
orbit_op_cost=100000;  %operating cost per satellites in orbit in $
holding_cost=1000000;  %holding cost per ground spare in $
manufac_cost=650000000;%manufacturing cost per satellite in $
fuel_cost=1000;        %cost for fuel for manuevering in $/kg
fixed_launch_cost=30000000;%fixed overhead cost for launching in $
variable_launch_cost=10000;%variable cost for launching in $/kg
discount_rate=.20/12;  %discount rate for simulation (monthly)
learning_rate=.95;     %learning rate for simulation (monthly)
base_image_val=25000000;  %value in dollars per Mkm^2 imaged
...
Reliability(1)=PNZ*((K*exp(-(1/theta1)^beta1))+((1-K)*exp(-(1/theta2)^beta2)));
for i=2:length(full_period)
    Reliability(i)=(PNZ*((K*exp(-((i)/theta1)^beta1))+((1-K)*exp(-((i)/theta2)^beta2))))/Reliability(i-1);
end
...
for MonteCarlo=1:NumMonteCarlos
    ...
    Revisit(1)=revisits_WorldView(3);
...
%% Failures
        ...
        %Manufacturing
        if (ground_spare_pop_array(t)<1 && (OpOrbitPop(t)+Tot_SEU_Failures(t))<(1.1*op_orbit_pop) && ((sum(sats.NumSats))+sum(sats_manu_start))<(1.1*op_orbit_pop)+sum(Lifecycle_Failures)+sum(Num_Radiation_Failures))
            sats_manu_start(t)=1;
        else
            sats_manu_start(t)=0;
        ...
        if ground_spare_pop_array(t)>=1  && (OpOrbitPop(t)+Tot_SEU_Failures(t))<(1*op_orbit_pop) && ((sum(launch_queue.OpNumLaunched))+sum(sats.NumSats))<(1*op_orbit_pop)+sum(Lifecycle_Failures)+sum(Num_Radiation_Failures)%Need to calculate launch costs separately if there is a launch to operational orbit AND parking orbit
            op_launch_start(t)=1;
