...
warning('off','all')
warning
load('D:\Materials\revisits_SkySat.mat');
revisits_array=fliplr(revisits_SkySat);
...
SatName='SkySat';
numSeeds=10000;
Component_Type=2;%1 = Space-Grade, 2 = Space-Tolerant, 3 = COTS - only used for SEUs

%Spacecraft and Orbit Parameters
sc_length=.6;        %spacecraft length in meters
sc_width=.6;         %spacecraft width in meters
sc_mass=110;         %spacecraft mass in kg
rad_tolerance=100;     %component radiation tolerance in krad
sc_shielding=80;       %spacecraft shielding in mils (must be between 5 and 400)
image_cap=27000;    %imaging capacity per satellite per day (km^2)
manufac_time=2;      %manufacturing time per satellite in months
...
op_orbit_pop=15;       %number of satellites in constellation at t=0
...
holding_cost=100000;  %holding cost per ground spare in $
manufac_cost=5000000;%manufacturing cost per satellite in $
fuel_cost=1000;        %cost for fuel for manuevering in $/kg
fixed_launch_cost=10000000;%fixed overhead cost for launching in $
variable_launch_cost=7500;%variable cost for launching in $/kg
discount_rate=.20/12;  %discount rate for simulation (monthly)
learning_rate=.85;     %learning rate for simulation (monthly)
base_image_val=500000;  %value in dollars per Mkm^2 imaged
...
for MonteCarlo=1:NumMonteCarlos
    ...
    sats.Radiation=[sum(cum_radiation(length(full_period)-27:end)),sum(cum_radiation(length(full_period)-18:end)),sum(cum_radiation(length(full_period)-9:end)),cum_radiation(length(full_period))]';
    sats.Status=repmat(["Active"]',length(NumSats),1);
    sats.Location=repmat(["Operational Orbit"]',length(NumSats),1);
    sats.Age=[27,18,9,0]';%in months
    ...
    for t=2:length(full_period)
        ...
        %Manufacturing
        if (ground_spare_pop_array(t)<3 && (OpOrbitPop(t)+Tot_SEU_Failures(t))<(1.1*op_orbit_pop) && ((sum(sats.NumSats))+sum(sats_manu_start))<(1.1*op_orbit_pop)+sum(Lifecycle_Failures)+sum(Num_Radiation_Failures))
            sats_manu_start(t)=1;
        else
            if mod(t,2)==0
            sats_manu_start(t)=1;
        ...
        if ground_spare_pop_array(t)>=4  && (OpOrbitPop(t)+Tot_SEU_Failures(t))<(1.2*op_orbit_pop) && (sum(launch_queue.OpNumLaunched)+sum(sats.NumSats))<(1.2*op_orbit_pop)+sum(Lifecycle_Failures)+sum(Num_Radiation_Failures)%Need to calculate launch costs separately if there is a launch to operational orbit AND parking orbit
            op_launch_start(t)=4;
        ...
        AreaOfCoverage(t)=(OpOrbitPop(t)+6).*AoC_sat/1000000;
        Revisit(t)=revisits_array(OpOrbitPop(t)+1);
        ...
        Holding_Costs_Array(t)=((OpOrbitPop(t)+6+ParkOrbitPop(t))*orbit_op_cost)+(ground_spare_pop_array(t)*holding_cost);
    ...
    Sat_Age_monte(MonteCarlo)=mean(sats.Age(1:4));
    ...
    if ~isempty(S4_Flags{seed})
        for i=1:length(S4_Flags{seed})
            ...
            Improvement_S4=find(Performance(S4_Flags{seed}(i):end)>=(0.95*Avg_Start_Performance));
    ...
    if ~isempty(S5_Flags{seed})
        ...
        Robustness_Max_monte=horzcat(Robustness_Max_monte,max(Robustness));
        Resilience_ShortTerm_S5=horzcat(Resilience_ShortTerm_S5,mean(Post_Performance)/Avg_Start_Performance);
        Improvement_S5=find(Performance(S5_Flags{seed}(i):end)>=(0.95*Avg_Start_Performance));
