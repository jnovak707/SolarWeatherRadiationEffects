function satdata=sgp4_file(t,fname)
...
while (1)
    ...
    satdata(ii).datetime = datetime([str2num(tline(19:20))+2000,1,1,0,0,0],'TimeZone','UTC')+days(str2num(tline(21:32))-1);
    datetime([str2num(tline(19:20))+2000,1,1,0,0,0],'TimeZone','UTC');
    days(str2num(tline(21:32)));
    % read second line
    tline = fgetl(fid);
    if ~ischar(tline)
        break
    ...
    satdata(ii).epoch = epoch;
    satdata(ii).norad_number = Cnum;
    satdata(ii).bulletin_number = ID;
    satdata(ii).classification = SC; % almost always 'U'
    satdata(ii).revolution_number = rNo;
    satdata(ii).ephemeris_type = Etype;
    satdata(ii).xmo = M * (pi/180);
    satdata(ii).xnodeo = raan * (pi/180);
    satdata(ii).omegao = omega * (pi/180);
    satdata(ii).xincl = i * (pi/180);
    satdata(ii).eo = e;
    satdata(ii).xno = no * TWOPI / MINUTES_PER_DAY;
    satdata(ii).xndt2o = TD1 * 1e-8 * TWOPI / MINUTES_PER_DAY_SQUARED;
    satdata(ii).xndd6o = TD2 * TWOPI / MINUTES_PER_DAY_CUBED;
    satdata(ii).bstar = BStar;
    
    tsince = -minutes(satdata(ii).datetime-t);
    satdata(ii).datetime_eval = t;
    [satdata(ii).pos, satdata(ii).vel] = sgp4(tsince, satdata(ii));
