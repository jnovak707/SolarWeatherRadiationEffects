function [SEU_Probs,Rad_Accum,S4_Flags,S5_Flags]=StormSeedGenerator_Final(sc_shielding,numSeeds,Component_Type,full_period,SatName);
%% Requirements:
...
load(append('D:\Materials\Storm_Doses_',SatName,'.mat'))%Loads shielding depths if not left over from BackgroundRadiation_Final
...
S4_Flags=cell(TotalSeeds,1);
S5_Flags=cell(TotalSeeds,1);
...
for ii=1:TotalSeeds
    ...
    for i=1:length(full_period) %for every month of every seed, we generate a number of expected events based on Kim et al.
        ...
        if expected_events_kim(i)<=0
            ...
            if sum(event_intensities{i}>=4)==1
                S4_Flags{ii}=horzcat(S4_Flags{ii},i); %So we can find cycles where an S4 was rolled later on
            end
            if sum(event_intensities{i}>=5)==1
                S5_Flags{ii}=horzcat(S5_Flags{ii},i); %So we can find cycles where an S5 was rolled later on
            end
        end
